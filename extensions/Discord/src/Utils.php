...
class DiscordUtils {
	...
	public static function handleDiscord ($emoji, $msg) {
		...
		$description = "$userProfile $descverb $pagemarkdown $rev";

		DeferredUpdates::addCallableUpdate( 
		function() use ( $stripped, $urls, $wgDiscordUseFileGetContents ) 
		{
			$user_agent = 'mw-discord/1.0 (github.com/jaydenkieran)';
			$json_data = [ 'content' => "$stripped" ];
			$date = date("Y-m-d H:i:s");
			$jsonObj->title = "";
			$jsonObj->description = $stripped;
			$jsonObj->url = "";
			$jsonObj->color = 14269203;
			$jsonObj->footer = array('text' => "SuperbossGames Wiki", 'icon_url' => "https://cdn.discordapp.com/emojis/587812105340846100.png?v=1");
			$jsonObj->timestamp = "$date";
		
		$json_data_embed  = array('content' => null, 'embeds' => array($jsonObj));
		
		$json = json_encode($json_data_embed);	
