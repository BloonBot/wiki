...
class DiscordHooks {
	...
	public static function onPageContentSaveComplete( &$wikiPage, &$user, $content, $summary, $isMinor, $isWatch, $section, &$flags, $revision, &$status, $baseRevId, $undidRevId ) {
		...
		$userProfile = DiscordUtils::createMarkdownLink($username, "https://intruderfps.com/agents/$user/");
		$rev = DiscordUtils::createRevisionText($revision);
		$descverb = wfMessage($msgKey);
		$pagemarkdown = DiscordUtils::createMarkdownLink($wikiPage->getTitle()->getText(), $wikiPage->getTitle()->getFullUrl( '', '', $proto = PROTO_HTTP ));
		
		$msg = "$userProfile $descverb $pagemarkdown $rev";
	...
	public static function onArticleDeleteComplete( &$article, User &$user, $reason, $id, $content, LogEntry $logEntry, $archivedRevisionCount ) {
		...
		$msg = wfMessage( 'discord-articledelete', DiscordUtils::createMarkdownLink($username, "https://intruderfps.com/agents/$user/"),
	...
	public static function onArticleUndelete( Title $title, $create, $comment, $oldPageId, $restoredPages ) {
		...
		$msg = wfMessage( 'discord-articleundelete', DiscordUtils::createUserLinks( $user->getRealname() ),
	...
	public static function onArticleRevisionVisibilitySet( &$title, $ids, $visibilityChangeMap ) {
		...
		$msg = wfMessage( 'discord-revvisibility', DiscordUtils::createUserLinks( $user->getRealname() ),
